...
class Session
{
    ...
    public static $data;
    public static $isDataSaved;
    public static $isDataChanged;
    public static $inited;
    public static $clientId;
    public static $userId;
    public static $isDisabled;
    public static $activePersonId;
    public static $activePerson;
    public static $login;
    public static $activePersonFirstName;
    public static $activePersonLastName;
    ...
    public static function init()
    {
        ...
        if (isset($_COOKIE['CLIENT_KEY'])) {
            ...
            $stmt1=$mysql->mysqli->prepare('SELECT u.LOGIN, c.CLIENT_ID, c.USER_ID, u.IS_DISABLED FROM sys_clients c, sys_users u WHERE u.USER_ID=c.USER_ID AND c.CLIENT_KEY=? AND c.REFRESH_TIME>(now()-INTERVAL '.self::TIMEOUT_CLIENT.' SECOND) ');
            if (!$stmt1) {
                trigger_error(\doq\t('SQL in client select query invalid'), E_USER_ERROR);
            }
            if (!$stmt1->bind_param('s', $givenClientKey)) {
                trigger_error(\doq\t('SQL in client select invalid'), E_USER_ERROR);
                $stmt1->close();
                return false;
            }
            if ($stmt1->execute() && $stmt1->bind_result($activePersonId, $login, $clientId, $userId, $isDisabled) && $stmt1->fetch()) {
                ...
                if ($clientId) {
                    ...
                    if (!$stmt1) {
                        trigger_error(\doq\t('Clients refreshener query to the database is invalid'), E_USER_ERROR);
                        return false;
                    }
                    $stmt1->bind_param('i', $clientId);
                    if (!$stmt1->execute()) {
                        trigger_error(\doq\t('Alarm! Table Client did not updated'), E_USER_ERROR);
                        return false;
                    }
                    $stmt1->close();

                    $stmt1=$mysql->mysqli->prepare('SELECT p.FIRST_NAME, p.LAST_NAME FROM sys_users u, sys_persons WHERE u.USER_ID=?');
                    if (!$stmt1->bind_param('i', $userId)) {
                        trigger_error(\doq\t('SQL in Person selection is invalid'), E_USER_ERROR);
                        $stmt1->close();
                        return false;
                    }
        ...
        if (!self::$clientId) {
            ...
            for ($j=0;$j<2;$j++) {
                ...
                if (!$stmt1) {
                    trigger_error(\doq\t('Looks like a client table schema was changed!'), E_USER_ERROR);
                    return false;
                }
        ...
        if (isset($_COOKIE['SESSION_KEY'])&&(!$shouldDropSession)) {
            ...
            if (!$stmt1) {
                trigger_error(\doq\t('Session refreshener query to the database is invalid'), E_USER_ERROR);
                return false;
            }
            $stmt1->bind_param('s', $givenSessionKey);
            if (!$stmt1->execute()) {
                trigger_error(\doq\t('Unable to get registered session data'), E_USER_ERROR);
                return false;
            }
            if (!$stmt1->bind_result($sessionId, $refreshTime, $sdata)) {
                trigger_error(\doq\t('Unable to get registered session data'), E_USER_ERROR);
                return false;
            }
            if ($stmt1->fetch()) {
                ...
                if (!$stmt1) {
                    trigger_error(\doq\t('Sessions refreshing request to the database is invalid'), E_USER_ERROR);
                    return false;
                }
                $stmt1->bind_param('ii', self::$clientId, $sessionId);
                if (!$stmt1->execute()) {
                    trigger_error(\doq\t('Problem with refreshing sessions table'), E_USER_ERROR);
                    return false;
                }
                $stmt1->close();
            } else {
        ...
        if (!self::$sessionId) {
            ...
            for ($j=0;$j<2;$j++) {
                ...
                if (!$stmt1) {
                    trigger_error(\doq\t('Looks like a session data schema was changed!'), E_USER_ERROR);
                    return false;
                }
